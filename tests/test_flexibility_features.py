...
def _load_dag_helpers():
    """Load helper functions from flexibility_features_dag.py without Airflow context."""
    stubs = {
    ...
    for parent in [Path(__file__).parent, Path(__file__).parents[1]]:
        dag_path = parent / "dags" / "processing_dags" / "flexibility_features_dag.py"
        if dag_path.exists():
    ...
    pytest.skip(
        "flexibility_features_dag.py not found — copy it to "
...
#  Lag feature construction (pure logic, no Firestore) 
...
class TestLagFeatureConstruction:
    ...
    def test_minimum_sessions_threshold(self):
        ...
        expected = max(0, len(df) - N_LAGS) if len(df) >= MIN_SESS else 0
        assert expected == 0
    ...
    def test_lag_ordering(self):
        N_LAGS = 5
        df     = self._make_user_df(n_sessions=10).reset_index(drop=True)
